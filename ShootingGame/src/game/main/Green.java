...
public class Green implements Runnable{
	...
	Green(double x,double y,Textures tex,Controller C){
		this.x=x;
		this.y=y;
		this.tex=tex;
		this.C=C;
...
void tick() {
		...
		for(int i=0;i<C.b.size();i++) {
			...
			if(this.getBound((int)x,(int) y).intersects(Temp.getBound((int)Temp.x, (int)Temp.y))) {
				if(Game.currentBonus.equals("Green")) {
					respawnFlag=true;
					numberOfhits+=1;
				...
				}else if(Game.currentEQN.equals("Green")) {
					respawnFlag=true;
					numberOfhits+=1;
					Game.BonusEQN=false;
					C.removeBullet(Temp);
	...
	void render(Graphics g) {
		g.drawImage(tex.green,(int)x,(int)y,null);	
	}
